<template>
  <section id="Projects-section">
    <span class="group-bouton">
      <button class="arrow-bouton prev" aria-label="Projets précédents">
        <svg
          aria-hidden="true"
          focusable="false"
          data-prefix="fas"
      ...
      <button class="arrow-bouton next" aria-label="Projets suivant">
        <svg
          aria-hidden="true"
          focusable="false"
          data-prefix="fas"
    ...
    <Splide class="slider" :options="splideOptions">
      <SplideSlide v-for="item in projectList" v-bind:key="item.id">
        <ProjectItem v-bind:project_item_prop="item" />
      </SplideSlide>
    </Splide>
  </section>
</template>
...
@media (max-width: 750px) {
  /** l'élément actif est allumé si on est sur mobile */
  .splide__slide.is-active {
    filter: grayscale(0) brightness(1);
    transform: scale(1.5, 1.5);
    -webkit-transform: scale(1, 1);
  }
  /* On desactive le hover */
  .splide__slide:hover {
    /*background-color: rgba(156, 49, 49, 0.3);*/
    filter: grayscale(0) brightness(1);
    border-radius: 7px;
